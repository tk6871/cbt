(function () {
  ...
  function renderLearningQuestion(question) {
    ...
    return `<article class="question-card ${imagePrimary ? 'image-primary' : ''}" id="question-${question.number}"><div class="question-meta"><span>${esc(subjectFor(s.round, question))}</span><button class="bookmark-button ${bookmarked ? 'active' : ''}" data-action="toggle-bookmark" data-id="${id}" title="북마크">★</button></div>${prompt}
      <div class="choice-list ${imagePrimary ? 'image-answer-list' : ''}">${question.choices.map((choice, index) => { const n = index + 1, cls = revealed ? (n === selected ? (n === question.answer ? 'correct' : 'wrong') : '') : n === selected ? 'selected' : ''; return `<button class="choice ${cls}" data-action="answer" data-number="${question.number}" data-choice="${n}" ${imagePrimary ? `aria-label="${n}번 선택"` : ''}><span>${imagePrimary ? n : CIRCLES[index]}</span>${imagePrimary ? '' : `<span>${choice.html || esc(choice.text)}</span>${renderImages(choice.images, '보기 이미지')}`}</button>`; }).join('')}</div>
  ...
  function renderExamQuestion(question) {
    ...
    return `${showSubject ? renderSubjectDivider(s.round, question, 'exam-section-title') : ''}<article class="exam-question ${held ? 'held' : ''} ${imagePrimary ? 'image-primary' : ''}" id="exam-question-${question.number}">${prompt}${imagePrimary ? `<button class="exam-image-hold" data-action="toggle-review" data-number="${question.number}" title="보류 표시">${held ? '★ 보류' : '☆ 보류'}</button>` : ''}<div class="exam-choices ${imagePrimary ? 'exam-image-answers' : ''}">${question.choices.map((choice, index) => `<button class="exam-choice ${selected === index + 1 ? 'selected' : ''}" data-action="answer" data-number="${question.number}" data-choice="${index + 1}" ${imagePrimary ? `aria-label="${index + 1}번 선택"` : ''}><span>${imagePrimary ? index + 1 : CIRCLES[index]}</span>${imagePrimary ? '' : `<span>${choice.html || esc(choice.text)}</span>${renderImages(choice.images, '보기 이미지')}`}</button>`).join('')}</div></article>`;
  }
  ...
  function changeHandler(event) {
    ...
    else if (key === 'visual-style') { setVisualStyle(event.target.value); route(); }
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') {
    ...
    navigator.serviceWorker.register('sw.js?v=247', { updateViaCache: 'none' })
      .then((registration) => {
