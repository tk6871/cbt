(function () {
  ...
  function setDynamicUiEnabled(enabled) {
    ...
    document.documentElement.dataset.dynamicUi = normalized ? 'on' : 'off';
    localStorage.setItem(DYNAMIC_UI_KEY, String(normalized));
  }
  ...
  function changeHandler(event) {
    ...
    else if (key === 'dynamic-ui') { setDynamicUiEnabled(event.target.value !== 'off'); route(); }
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') {
    ...
    navigator.serviceWorker.register('sw.js?v=250', { updateViaCache: 'none' })
      .then((registration) => {
