...
- 이번 문서에는 전 종목 이미지 개선, 선택형 테마·동적 화면 전환과 v2.4.7 CBT 후속 기능 상태가 포함됨
...
- 패치노트 v2.4.7과 Service Worker 캐시 v250까지 반영했습니다.
- 기존 CBT 화면을 기본 UI로 유지하면서 설정 화면에서만 기본·심슨·선재 테마를 선택하도록 구성했습니다. 상단과 신기능 체험실의 즉시 UI 전환 버튼은 제거했습니다.
...
- 류선재 단독 장면 6장과 변우석 일상·SNS 사진 4장을 선재 테마의 홈·로고·데스크톱·모바일 메뉴에 적용하고 자동 순환하도록 구성했습니다.
- 선재 사진 교체 간격은 `unified-cbt-sunjae-rotation-seconds`에 저장하며 끔·5초·10초·30초·1분·3분·5분을 선택할 수 있습니다. 저장값이 없을 때 기본은 10초입니다.
- 심슨 메뉴도 호머·바트 본편 사진과 기능 배지를 함께 표시하고, `King-Size Homer`의 1430×1080 스틸을 추가 업스케일링 없이 사용합니다.
...
- v2.4.7 `npm run typecheck` 통과
- v2.4.7 `npm run build` 통과
- v2.4.7 `npm run audit:questions` 통과: 9개 종목, 248회차, 19,610문제, 고유 이미지 참조 4,126개, 구조 오류 0, 보기 수 경고 0, 누락 이미지 0
- 이미지가 있으나 해설이 없는 948문제는 실제 원본·업스케일·정답·해설 육안 대조 후보이며 아직 전체 수동 검수가 끝난 상태가 아님
- v2.4.7 `node --check sw.js`, `node --check data/changelog-vue.js`, `node --check app.js`, `node --check calculator.js`, `git diff --check` 통과
- 로컬 Chrome에서 상단 UI 전환 버튼이 사라지고 설정에만 기본·심슨·선재 선택이 남은 것을 확인
- 로컬 Chrome에서 선재 테마의 홈·로고·사이드 메뉴 사진을 확인하고 5초 설정에서 다음 사진으로 자동 변경되는 동작을 검증한 뒤 10초로 복원
- 설정 화면에 끔·5초·10초·30초·1분·3분·5분 선택지가 표시되는 것을 확인
- 실제 태블릿·모바일 기기의 터치 검증은 아직 수행하지 않았으며 데스크톱 Chrome과 반응형 CSS로 확인함
...
- v2.4.6 GitHub Pages 배포는 Push 후 약 11~14분 지연 뒤 성공했고 공개 `sw.js` 캐시는 v249로 갱신됨. v2.4.7·캐시 v250은 이번 커밋 Push 후 배포 대기 상태
...
`CBT수정안.docx`의 필수 기능과 후속 계산문제·공조 암기장·테마·베타 UI의 코드 구현을 완료했습니다. 다음 우선 작업은 v2.4.7 커밋을 Push하고 Pages 배포를 확인한 뒤, 실제 태블릿·모바일에서 선재/심슨 사진 메뉴·결과 배너와 베타 UI 3종을 비교하는 것입니다. 이후 이미지 문제·해설 없음 948건을 원본과 업스케일 이미지로 회차별 수동 검수해야 합니다.
